 ...
 * authorize
 */
public class ActionUpdateKsn extends BaseAction {

    public ActionUpdateKsn() {
        super(GetString.getString(KivviApplication.getContext(),R.string.actionPinpad)+"|"+ GetString.getString(KivviApplication.getContext(),R.string.LoadKey)+"|"+ GetString.getString(KivviApplication.getContext(),R.string.DUKPT)+"|"+GetString.getString(KivviApplication.getContext(),R.string.initkeyenbykpk)+"|"+ GetString.getString(KivviApplication.getContext(),R.string.Update_KSN)+"-4");
    }
    ...
    public void myAction(String actionName) {
        ...
        try {
            ...
            kvDevPinpad.Set(KV.KEY.PINPAD_KSN.REQUIRE.OPERATE, "UPDATE");

            int ret = kvDevPinpad.Action(KV.CMD.PINPAD_KSN);
            if(ret == KV.RET.OK){
                byte[] ksn = kvDevPinpad.GetByteArray(KV.KEY.PINPAD_KSN.RESPONSE.KSN);

                setMsg("(AppID = 2)取KSN成功：" + F.ByteArrayToHexString(ksn));
            }else{
                String errMsg = "(AppID = 2)读取KSN失败" + ret + "\n";
                try {
